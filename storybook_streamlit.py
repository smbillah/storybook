...
- character_bible must include stable visual details: age, hairstyle, hair color, signature outfit, recurring accessories, facial expression style.
...
- continuity_prompt should preserve the same character identity, outfit logic, palette, rendering style, and storybook look.
...
def create_storybook_pdf() -> bytes:
    ...
    for idx, page in enumerate(pages, start=1):
        ...
        body_lines = wrap_text(draw, page.get("text", ""), body_font, int(page_width * 0.7))
    ...
    buffer = BytesIO()
    pdf_images[0].save(buffer, format="PDF", save_all=True, append_images=pdf_images[1:])
    buffer.seek(0)
    return buffer.getvalue()


def save_generated_book_to_books(books_root: Path) -> str:
    summary = st.session_state.summary_data or {}
    pages_data = st.session_state.pages_data or {}
    image_paths = st.session_state.image_paths or []

    title = summary.get("title", "Untitled Storybook")
    slug = slugify(title)
    book_dir = books_root / slug
    book_dir.mkdir(parents=True, exist_ok=True)

    page_ids = []
    for idx, page in enumerate(pages_data.get("pages", []), start=1):
        page_id = f"{idx:03d}"
        page_ids.append(page_id)

        (book_dir / f"{page_id}.md").write_text(page.get("text", ""), encoding="utf-8")

        if idx - 1 < len(image_paths):
            src = Path(image_paths[idx - 1])
            if src.exists():
                ext = src.suffix.lower() or ".png"
                shutil.copyfile(src, book_dir / f"{page_id}{ext}")
    ...
    st.session_state["last_saved_book_slug"] = slug
    st.session_state["library_selected_book"] = slug
    return slug
...
def reset_project() -> None:
    ...
    st.session_state.last_saved_book_slug = None


# ============================================================
# MAIN RENDER FUNCTION
# ============================================================
def render_storybook_generator(books_root: Path = Path("Books")) -> None:
    init_state()
        ...
        if st.button("Generate Full Storybook", use_container_width=True):
            try:
                ...
                with st.status("Building full storybook: pages, prompts, and images..."):
                    generate_full_book()
                    saved_slug = save_generated_book_to_books(books_root)
                st.success(f"Full storybook generated and added to the library: {saved_slug}")
            except Exception as exc:
        ...
        if total_pages > 0:
            ...
            if not st.session_state.edit_mode:
                ...
                with left_arrow:
                    ...
                    if st.button("←", use_container_width=True, disabled=st.session_state.selected_page <= 1, key="generator_prev_page"):
                ...
                with right_arrow:
                    st.markdown("<div style='height:260px;'></div>", unsafe_allow_html=True)
                    if st.button("→", use_container_width=True, disabled=st.session_state.selected_page >= total_pages, key="generator_next_page"):
                ...
                with edit_col1:
                    if st.button("Save Page Text", use_container_width=True):
                        ...
                        save_generated_book_to_books(books_root)
                ...
                if st.session_state.image_prompts_data:
                    with st.expander("Character and style continuity settings"):
                        ...
                        st.write(st.session_state.summary_data.get("character_bible", ""))
                        st.write("**Style bible:**")
                        st.write(st.session_state.summary_data.get("style_bible", ""))